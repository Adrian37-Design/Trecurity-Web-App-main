...
<script setup lang="ts">
        ...
        try {
            ...
            try {
                ...
                if (data) {
                    ...
                    // Fix: Handle nested user object from direct login vs flat object from verify-otp
                    if (data.user) {
                        setUser(data.user);
                    } else {
                        setUser(data);
    ...
    const loadRecaptcha = async () => {
        if(process.env.NODE_ENV === 'production') {
            ...
            // Force hardcoded key if runtime config is invalid or placeholder
            let SITE_KEY = RECAPTCHA_CLIENT_SITE_KEY;
            const HARDCODED_KEY = '6LfB-UMsAAAAAErpo7GNwefulO-wNmTI6HpEZ8td';
            
            if (!SITE_KEY || SITE_KEY === '' || SITE_KEY === 'temp-key' || SITE_KEY === 'dummy-recaptcha-key') {
                SITE_KEY = HARDCODED_KEY;
            }

            // If it's still invalid after fallback, skip
            if (!SITE_KEY || SITE_KEY === '' || SITE_KEY === 'temp-key') {
