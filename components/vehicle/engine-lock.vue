...
<script setup lang="ts">
    ...
    // Clear loading when props update (parent refreshed data after command)
    watch(() => props.vehicle?.tracking_data?.at(0)?.updated_at, () => {
        is_loading.value = false;
    })

    const toggleEngineLock = () => {
        // Capture intended command BEFORE any async to avoid race condition
        const currentlyLocked = is_engine_locked();
        const commandCode = currentlyLocked ? 'ENGINE_UN_LOCK' : 'ENGINE_LOCK';
        const actionLabel = currentlyLocked ? 'unlock' : 'lock';

        Swal.fire({
            icon: 'question',
            title: currentlyLocked ? 'Unlock Engine' : 'Lock Engine',
            text: `Do you want to ${ actionLabel } this vehicle's engine`,
            showCancelButton: true
        }).then(async (result)=>{
            if(result.isConfirmed){  
                is_loading.value = true;

                const response = await $fetch<{ data: Geofence, message: string, success: boolean }>('/api/command/engine', {
                    method: 'POST',
                ...
                if(response.success) {
                    toast.add({ severity: 'success', summary: 'Engine Command', detail: `The engine ${ actionLabel } command was successfully sent to the server. This might take a few minutes depending on vehicle connectivity.`, life: 15000})
                    // Keep loading=true until parent refreshes and clears it via prop change
                    emit('onSendEngineCommand', true)
                    // Fallback: clear loading after 5 seconds in case parent doesn't refresh
                    setTimeout(() => { is_loading.value = false; }, 5000);
                } else {
                    // On failure, clear loading immediately
                    is_loading.value = false;
