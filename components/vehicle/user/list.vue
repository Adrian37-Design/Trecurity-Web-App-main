    ...
    let columns = ref([
        ...
        },
        {
            ...
            render: (_, __, vehicle: Vehicle) => {
                return `
                    <button id="btn-view-${ vehicle.number_plate }" title="View ${ vehicle.number_plate }" class="p-button p-component p-button-icon-only p-button-rounded p-button-text p-button-info" type="button" aria-label="Edit ${ vehicle.number_plate }">
    ...
    let options = ref({
        ...
        drawCallback: ({ json: { data }}) => {
            $('button:regex(id, btn-view-*)').on('click', (ev)=>{
                ...
                if (number_plate) {
                    navigateTo(`/vehicles/${number_plate}`)
                }
            })
        }
    });
