...
<script setup lang="ts">
    ...
    const upsert = async () => {
        try {
            ...
            if (!name.value) {
                toast.add({ severity: 'warn', summary: 'Validation', detail: 'Company name is required', life: 5000 });
                return;
            }
            if (!email.value) {
                toast.add({ severity: 'warn', summary: 'Validation', detail: 'Company email is required', life: 5000 });
                return;
            }
            if (!phone.value) {
                toast.add({ severity: 'warn', summary: 'Validation', detail: 'Company phone number is required', life: 5000 });
                return;
            }

            if (!current_data) {
                if (!initial_admin_name.value) {
                    toast.add({ severity: 'warn', summary: 'Validation', detail: 'Initial admin name is required', life: 5000 });
                    return;
                }
                if (!initial_admin_surname.value) {
                    toast.add({ severity: 'warn', summary: 'Validation', detail: 'Initial admin surname is required', life: 5000 });
                    return;
                }
                if (!initial_admin_email.value) {
                    toast.add({ severity: 'warn', summary: 'Validation', detail: 'Initial admin email is required', life: 5000 });
                    return;
                }
                if (!initial_admin_phone.value) {
                    toast.add({ severity: 'warn', summary: 'Validation', detail: 'Initial admin phone number is required', life: 5000 });
                    return;
                }
            }
            ...
            if(success) {
                ...
                if(message?.toString().includes("<no response> Failed to fetch") || message?.toString().includes("net::ERR")) {
                    ...
                    toast.add({ severity: 'warn', summary: 'Oops!', detail: message || "An internal application error has occurred. Please try to refresh your page and start again.", life: 8000})
                }
            }
        } catch (error) {
            console.error(error)
