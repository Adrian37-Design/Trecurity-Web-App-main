<template>
    <div ref="map_el">
        ...
        <div class="text-center my-4">
            <SelectButton v-model="geofenceMethod" :options="Object.values(GeofenceMethod)" :disabled="user.approval_level !== 'SUPER_ADMIN' && user.approval_level !== 'MASTER_ADMIN'" :allowEmpty="false" />
        </div>
...
<script setup lang="ts">
    ...
    const syncWithVehicle = () => {
        if (props.vehicle) {
            console.log("DEBUG: Child syncWithVehicle called", JSON.stringify(props.vehicle.geofence_alert_recipients));
            // @ts-ignore
            const rawRecipients = props.vehicle.geofence_alert_recipients;
            recipients.value = Array.isArray(rawRecipients) ? [...rawRecipients] : [];
            lock_engine_on_geofence_violation.value = props.vehicle.lock_engine_on_geofence_violation ?? false;
        }
    }

    // Use a watchEffect or immediate watch to ensure sync on mount and prop changes
    ...
    onMounted(() => {
        if (vehicle.value?.geofence) {
            // @ts-ignore
            geometry.value = vehicle.value.geofence.geometry;
    ...
    const addGeofence = async (code?: 'create' | 'edit' | 'delete') => {
        ...
        try {
            ...
            if (code === 'delete') {
                geometry.value = [];
                selectedRoute.value = null;
            } else if (geofenceMethod.value === GeofenceMethod.Custom) {
                selectedRoute.value = null;
            } else if (geofenceMethod.value === GeofenceMethod.Route) {
                geometry.value = [];
            }
