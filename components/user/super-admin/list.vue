<template>
    ...
    <Dialog v-model:visible="showCreateUserDialog" modal header="Create User" :style="{ width: '50vw' }">
        <DialogUserSuperAdminUpsert v-if="user.approval_level === 'SUPER_ADMIN'" @reloadTable="reloadTable" />
    </Dialog>
    <Dialog v-model:visible="showEditUserDialog" modal header="Edit User" :style="{ width: '50vw' }">
        <DialogUserSuperAdminUpsert v-if="user.approval_level === 'SUPER_ADMIN'" :current_data="edit_user_data" @reloadTable="reloadTable" />
    </Dialog>
</template>
