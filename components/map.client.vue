    ...
    <div ref="map_el">
        <div>
            <div class="row d-flex justify-content-center mb-1 p-1">
                ...
                <!-- TRACKING CONTROLS -->
                <div v-if="option === 'REALTIME'" class="col-md-12 d-flex justify-content-center mt-2 align-items-center gap-2">
                    <Button 
                        @click="toggleTracking" 
                        :severity="isTracking ? 'danger' : 'success'" 
                        :icon="isTracking ? 'ti ti-player-stop' : 'ti ti-broadcast'" 
                        :label="isTracking ? (isOnline ? 'Broadcasting' : 'Recording Offline') : 'Start Tracking'" 
                        :loading="isSyncing"
                        rounded 
                        size="small"
                        :disabled="!current_vehicle"
                    />
                    <span v-if="queueSize > 0" class="p-tag p-component p-tag-warning" v-tooltip="'Data points waiting to upload'">
                        <i class="ti ti-database me-1"></i> {{ queueSize }} Sync Pending
                    </span>
                     <span v-if="error" class="text-danger text-xs ms-2">{{ error }}</span>
                </div>
            </div>
        </div>
        <div class="card">
...
<script setup lang="ts">
    ...
    // TRACKER LOGIC
    const { isTracking, startTracking, stopTracking, error } = useTracker();
    const { queueSize, isOnline, isSyncing } = useOfflineSync();

    const toggleTracking = () => {
        if (isTracking.value) {
            stopTracking();
            toast.add({ severity: 'info', summary: 'Tracking Stopped', life: 3000 });
        } else {
            console.log("Starting tracking for", current_vehicle.value);
            if (current_vehicle.value) {
                startTracking(current_vehicle.value.id);
                toast.add({ severity: 'success', summary: 'Tracking Started', detail: 'Location is being recorded in background', life: 3000 });
            } else {
                toast.add({ severity: 'error', summary: 'Error', detail: 'Please select a vehicle first', life: 3000 });
            }
        }
    }
    ...
    const getVehicleHistory = async (id: string, isBackgroundError=false) => {
        ...
        try {
            ...
            if(data.length > 0) {
                ...
                // Draw polyline regardless of mode
                drawHistoryPolyline(history_markers.value)
            } else {
    ...
    const getIcon = (vehicle_type: string) => {
        ...
        if (vehicle_type === "EXCAVATOR") return "backhoe";
    ...
    const getAddress = async (lat: number, lon: number, elementId: string) => {
        const el = document.getElementById(elementId);
        if(!el) return;
        
        // Check current text to avoid re-fetching if already loaded (unless "Loading...")
        if(el.innerText !== 'Loading...' && el.innerText !== 'Address unavailable') return;

        const key = `${lat.toFixed(5)},${lon.toFixed(5)}`;
        if(addressCache.has(key)) {
            el.innerText = addressCache.get(key) || 'Address unavailable';
            return;
        }

        try {
            // Delay slightly to allow popup animation
            // await new Promise(r => setTimeout(r, 100));

            const response = await fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lon}&zoom=18&addressdetails=1`, {
                headers: {
                    'User-Agent': 'Trecurity/1.0 (contact@trecurity.com)'
                }
            });
            ...
            if(a) {
                ...
                if(a.suburb || a.neighbourhood || a.district) parts.push(a.suburb || a.neighbourhood || a.district);
                else if(a.city || a.town || a.village || a.county) parts.push(a.city || a.town || a.village || a.county);
            }
            
            const addr = parts.length > 0 ? parts.join(', ') : (data.display_name?.split(',').slice(0, 2).join(',') || 'Address unavailable');
            ...
            if(document.getElementById(elementId)) el.innerText = 'Address unavailable';
        }
    }
    ...
    const updateHistoryPlayback = (tracking_data: TrackingData & { vehicle: Vehicle & { user: User[], company: Company }, geofence: Geofence }, current_index: number) => {
        const { lat, lon, satellites, geofence_violation_state, is_engine_locked, altitude, speed, fuel_level, battery_percentage, external_battery_voltage, ignition, state, time_from, time_to, course, signal_strength, operator_name, updated_at } = tracking_data;
        const vehicle = tracking_data?.vehicle;
        
        // Skip if no vehicle data
        if (!vehicle) return;
        
        const geofence = vehicle?.geofence;
        ...
        const popup_contents = `
                ...
                ${ user.value.approval_level === 'SUPER_ADMIN' || user.value.approval_level === 'MASTER_ADMIN' ? `<div class="d-flex justify-content-between">
                    <span>Company</span>
                    <span>${ vehicle.company.name }</span>
                </div>` : '' }
                <div class="d-flex justify-content-between mt-1">
                    <span class="py-1">Address</span>
                    <span class="p-tag p-component p-tag-secondary">
                        <span class="p-tag-value" id="address-playback">Loading...</span>
                    </span>
                </div>
                <div class="d-flex justify-content-between mt-1">
                ...
                <div class="d-flex justify-content-between mt-1">
                    ...
                    <div style="margin-left: 10px;">
                        <div>Device: ${ battery_percentage ? battery_percentage + '%' : 'No Data' }</div>
                        <div>Vehicle: ${ external_battery_voltage ? external_battery_voltage + 'V' : 'No Data' }</div>
                    </div>
                </div>
                <div class="d-flex justify-content-between mt-1">
                    <span>Ignition</span>
                ...
                <div class="d-flex justify-content-between mt-1">
                    ...
                    <span class="p-tag p-component p-tag-info">
                        <span class="p-tag-value">${ moment(time_to).format('ddd, DD MMM yy, h:mm a') }</span>
                    </span>
                </div>
        ...
        if(!history_playback_marker) {
            ...
            history_playback_marker.on('popupopen', () => {
                getAddress(lat, lon, 'address-playback');
            });
        } else {
            // Update marker location
            history_playback_marker.setLatLng([lat, lon]) 
            
            // Should verify if popup is open and update address live?
            // For playback, it changes fast, maybe just let it be. 
            // If user pauses and opens popup, next time it opens it will trigger.
            // But since we use 'popupopen', it triggers when opened.
            history_playback_marker.off('popupopen'); // Remove old listener
            history_playback_marker.on('popupopen', () => {
                getAddress(lat, lon, 'address-playback');
            }); 
        ...
        //@ts-ignore
        if(geofence?.geometry && Array.isArray(geofence.geometry) && geofence.geometry.length > 0) {
            if(polygon?.isEmpty()) {
                //@ts-ignore
    ...
    let history_polyline: L.Polyline;

    const drawHistoryPolyline = (data: TrackingData[]) => {
         if(history_polyline) map.removeLayer(history_polyline);
         const latlngs = data
            .filter(d => d.lat !== null && d.lon !== null && d.lat !== 0 && d.lon !== 0) // Filter invalid coordinates
            .map(d => [d.lat, d.lon]);
            
         if(latlngs.length > 0) {
             history_polyline = L.polyline(latlngs, { color: 'blue', weight: 4, opacity: 0.7 }).addTo(map);
         }
    }
    ...
    const updateHistoryClusterMarkers = (tracking_data: (TrackingData & { vehicle: Vehicle & { company: Company } })[]) => {
        ...
        for (let index = 0; index < tracking_data.length; index++) {
            const { lat, lon, satellites, geofence_violation_state, is_engine_locked, altitude, speed, fuel_level, battery_percentage, external_battery_voltage, ignition, state, time_from, time_to, course, signal_strength, operator_name, vehicle, updated_at } = tracking_data[index];
            ...
            const marker = L.marker([lat, lon], { icon: marker_html }).bindPopup(`
                    ...
                    <div class="d-flex justify-content-between mt-1">
                        <span class="py-1">Address</span>
                        <span class="p-tag p-component p-tag-secondary">
                            <span class="p-tag-value" id="address-history-${ index }">Loading...</span>
                        </span>
                    </div>
                    <div class="d-flex justify-content-between mt-1">
                        <span class="py-1">No. Plate</span>
            ...
            marker.on('popupopen', () => {
                getAddress(lat, lon, `address-history-${index}`);
            });
    ...
    const updateRealtimeMarkers = (tracking_data: RealTimeData[]) => {
        ...
        for (let index = 0; index < tracking_data.length; index++) {
            ...
            const { last_seen, type, company, number_plate, user: users, configuration } = item;
            ...
            const external_battery_voltage = (configuration as any)?.external_battery_voltage || item.tracking_data?.external_battery_voltage;
            ...
            const marker = L.marker([lat, lon], { icon: marker_html }).bindPopup(`
                <main style="width: 250px">
                    ...
                    <div class="d-flex justify-content-between mt-1">
                        <span class="py-1">Address</span>
                        <span class="p-tag p-component p-tag-secondary">
                            <span class="p-tag-value" id="address-realtime-${ number_plate }">Loading...</span>
                        </span>
                    </div>
                    <div class="d-flex justify-content-between mt-1">
                        <span class="py-1">No. Plate</span>
                    ...
                    <div class="d-flex justify-content-between mt-1">
                        ...
                        <span>${ external_battery_voltage ? external_battery_voltage + 'V' : 'No Data' }</span>
                    </div>
                    <div class="d-flex justify-content-between mt-1">
                        <span>Ignition</span>
                        <span>${ ignition === null || ignition === undefined ? 'No Data' : ignition ? 'On' : 'Off' }</span>
            ...
            marker.on('popupopen', () => {
                getAddress(lat, lon, `address-realtime-${ number_plate }`);
            });
    ...
    watch(map_is_visible, () => {
        setTimeout(() => {
            if(!map) {
                ...
                if(props.vehicle?.geofence && props.vehicle.geofence.geometry && Array.isArray(props.vehicle.geofence.geometry)) {
                    //@ts-ignore
                    if(polygon) {
