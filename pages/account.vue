                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-md-12 row">
                                                <div class="col-md-4 mb-3">
                                                    <Password v-model="old_password" :feedback="false" toggleMask placeholder="Old Password" />
                                                </div>
                                                <div class="col-md-4 mb-3">
                                                    <Password v-model="new_password" toggleMask placeholder="New Password" >
                                                        <template #header>
                                                ...
                                                <div class="col-md-4 mb-3">
                                                    <Password v-model="new_confirm_password" :feedback="false" toggleMask :class="{ 'p-invalid': new_password && !new_confirm_password || new_password != new_confirm_password }" placeholder="Confirm Password" />
                                            ...
                                            <div class="d-flex mb-3">
                                                <div class="m-3">
                                                <Button label="Update" :class="{ 'p-button-secondary': !name || !surname || !phone || (new_password && (!old_password || !new_confirm_password || new_password !== new_confirm_password)), 'p-button-success': name && surname && phone && (!new_password || (old_password && new_confirm_password && new_password === new_confirm_password)) }" type="submit" />
                                                </div>
                                            </div>
                                        </div>
                                    </form>
...
<script setup lang="ts">
    ...
    const update = async () => {
        ...
        if (new_password.value || old_password.value || new_confirm_password.value) {
            if (!old_password.value) {
                toast.add({ severity: 'warn', summary: 'Missing Field', detail: 'Old Password is required to change your password', life: 5000 });
                return;
            }
            if (!new_password.value) {
                toast.add({ severity: 'warn', summary: 'Missing Field', detail: 'New Password is required', life: 5000 });
                return;
            }
            if (new_password.value !== new_confirm_password.value) {
                toast.add({ severity: 'warn', summary: 'Validation Error', detail: 'New Password and Confirm Password do not match', life: 5000 });
                return;
            }
        }
