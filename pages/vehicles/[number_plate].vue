                ...
                <div class="block">
                    <div class="d-flex justify-content-between mb-4">
                        <div class="d-flex fs-6">
                            ...
                            <Button @click="recharge_dialog.open()" icon="ti ti-bolt" rounded severity="warning" class="ms-2" title="Recharge Airtime" :disabled="is_loading" />
                            <span v-if="is_loading" title="Loading data">
                                <i class="pi pi-spinner pi-spin fs-6 mt-2 ms-2"></i>
                            </span>
                        </div>
                        <DialogVehicleRecharge ref="recharge_dialog" :vehicle="vehicle" @close="getVehicle(true)" />
...
<script setup lang="ts">
    ...
    const getVehicle = async (rerender_main_map = false, silent_update = false) => {
        ...
        try {
            ...
            if (!result.success) {
                toast.add({ severity: 'error', summary: 'Access Error', detail: result.message, life: 5000 });
